...
class ProtagonistaDisfrazado: 
    ...
    def recuperar_casas_DFS_iterativo(self, casa_inicial: Casa )->list:
        ...
        #COMPLETAR
        lista = []
        cola = deque([casa_inicial])
        distancia = 0

        while cola:
            casa_actual = cola.pop()
            if casa_actual is not None:
                distancia += casa_actual.distancia
                cola.append(casa_actual.hijo_izquierdo)
                cola.append(casa_actual.hijo_derecho)
                diccionario = {'casa': casa_actual, 'distancia': distancia}
